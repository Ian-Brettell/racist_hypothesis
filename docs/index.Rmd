...
## Show distribution of median and mean Fst
```{r}
# Median
hits_filt %>% 
  dplyr::group_by(TRAIT) %>% 
...
## Get their ranks and percentiles
...
### Calculate KS test comparing between the distributions of hits and controls
...
```{r, fig.cap = "D statistic from KS test comparing distributions of hits vs controls", out.width='150%'}
# Pull out statistic
ks_stats = purrr::map_dfr(ks_out, function(TRAIT){
  TRAIT$statistic
}, .id = "TRAIT")
...
#### Bonferroni correction and add stars
...
# Rank by difference from controls
