...
# How many traits have more than 1 SNP in their raw associations?
nrow(raw_counts)
```
...
rank_pal = turbo(n = length(rank_fst))
names(rank_pal) = rank_fst
...
median_fst = hits_filt %>% 
  ...
  dplyr::arrange(MEDIAN_FST) %>%
  # add negative log
  dplyr::mutate(MEDIAN_FST_ADJ = dplyr::if_else(MEDIAN_FST <= 0, 1e-3, MEDIAN_FST),
...
log_bins = seq(min(median_fst$MEDIAN_FST_NL),
               max(median_fst$MEDIAN_FST_NL),
               length.out = n_cols + 1)
